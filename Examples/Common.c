...
int CommonInit(Context *context, SDL_WindowFlags windowFlags) {
  context->Device =
      SDL_CreateGPUDevice(SDL_GPU_SHADERFORMAT_WGSL, true, "webgpu");
...
SDL_GPUShader *LoadShader(SDL_GPUDevice *device, const char *shaderFilename,
  ...
  SDL_snprintf(fullPath, sizeof(fullPath),
               "/Content/Shaders/Compiled/WGSL/%s.wgsl", shaderFilename);
  format = SDL_GPU_SHADERFORMAT_WGSL;
  entrypoint = "main";

  SDL_LogError(SDL_LOG_CATEGORY_GPU, "Loading shader from path %s", fullPath);
  ...
  SDL_GPUShaderCreateInfo shaderInfo = {.code = code,
                                        .code_size = codeSize,
...
CreateComputePipelineFromShader(SDL_GPUDevice *device,
  ...
  SDL_GPUComputePipeline *pipeline =
      SDL_CreateGPUComputePipeline(device, &newCreateInfo);
  if (pipeline == NULL) {
...
SDL_Surface *LoadImage(const char *imageFilename, int desiredChannels) {
  ...
  SDL_snprintf(fullPath, sizeof(fullPath), "/Content/Images/%s", imageFilename);
...
void *LoadASTCImage(const char *imageFilename, int *pWidth, int *pHeight,
  ...
  *pImageDataLength = block_count_x * block_count_y * 16;
  ...
  SDL_memcpy(data, (char *)fileContents + sizeof(ASTCHeader),
             *pImageDataLength);
...
void *LoadDDSImage(const char *imageFilename, SDL_GPUTextureFormat format,
                   int *pWidth, int *pHeight, int *pImageDataLength) {
  char fullPath[256];
  SDL_snprintf(fullPath, sizeof(fullPath), "/Content/Images/%s", imageFilename);
  ...
  bool hasDX10Header =
      header->ddspf.dwFlags == 0x4 && header->ddspf.dwFourCC == 0x30315844;
  ...
  *pImageDataLength = header->dwPitchOrLinearSize;
  ...
  void *data = SDL_malloc(*pImageDataLength);
  SDL_memcpy(data,
             (char *)fileContents + sizeof(DDS_HEADER) +
                 (hasDX10Header ? sizeof(DDS_HEADER_DXT10) : 0),
             *pImageDataLength);
  SDL_free(fileContents);
